...
class App extends Component {
  state = {
    isSelected: tabsList[0].tabId,
    clickedImage: imagesList[0].id,
    userScore: 0,
    timerTime: 60,
  }
  ...
  filteredImages = () => {
    ...
    return imagesList.filter(eachItem => eachItem.category === isSelected)
  }
  ...
  markTheScore = id => {
    ...
    if (clickedImage === id) {
      this.setState(prevState => ({
        userScore: prevState.score + 1,
        clickedImage: randomImage.id,
      }))
    } else {
      this.setState({timerTime: 1})
    }
  }

  displayThumbnail = () => {
    const {clickedImage} = this.state
    return imagesList.filter(eachItem => eachItem.id === clickedImage)
  }

  reduceTime = () => {
    this.setState(prevState => {
      const newTime = prevState.timerTime - 1
      if (newTime <= 0) {
  ...
  getTheItem = tabId => {
    this.setState({isSelected: tabId})
  }

  playTheGame = () => {
    const {isSelected, clickedImage, timerTime} = this.state
    const filteredImages = this.filteredImages()
    console.log(clickedImage)
    console.log(timerTime)
    const desiredThumbnail = this.displayThumbnail()
    return timerTime > 0 ? (
      <>
        <ul className="thumbnail-container">
          <img
            src={desiredThumbnail[0].imageUrl}
            alt="match"
            className="thumbnail"
          />
        </ul>
        <ul className="tabs-list">
          {tabsList.map(eachItem => (
            <CategoryItem
              ...
              key={eachItem.tabId}
              isSelected={isSelected === eachItem.tabId}
              getTheItem={this.getTheItem}
            />
          ))}
        </ul>
        <ul className="images-container">
          {filteredImages.map(eachItem => (
            <CategoryImages
              ...
              key={eachItem.id}
              markTheScore={this.markTheScore}
            />
          ))}
        </ul>
      </>
  ...
  render() {
    const {clickedImage, userScore, timerTime} = this.state
          ...
          <ul>
            ...
            <li className="score-container">
              ...
              <img
                ...
                className="timer"
              />
              <p className="score">{timerTime} Sec</p>
            </li>
          </ul>
        </nav>
        {this.playTheGame()}
